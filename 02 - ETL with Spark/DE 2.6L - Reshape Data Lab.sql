...
-- Eventhough the result is grouped by user_id, in this case
-- we do not use group by which I found weird
-- pivot function itself groups by the selected columns
...
-- MAGIC %python
-- MAGIC # TODO
-- MAGIC events_pivot_df = (spark.read.table("events")
...
-- MAGIC events_pivotDF = spark.table("events_pivot")
-- MAGIC clickpathsDF = events_pivotDF.join(transactionsDF, events_pivotDF.user == transactionsDF.user_id)
