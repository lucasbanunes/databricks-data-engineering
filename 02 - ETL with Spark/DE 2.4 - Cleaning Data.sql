...
-- MAGIC display(usersDF.where(col("email").isNull()).count())
...
-- Groupes deduped_users by user_id and counts how many rows are in each group 
-- Afterwards validates if the max row_count is no bigger than one
-- This means that all groups have no more than one row
...
-- Same as before
-- Groups by email and tests if the max number of ids is bigger than one 
